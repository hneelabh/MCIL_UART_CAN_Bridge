...
#define BT_UART_RX_PIN  1       // GP1 = Pico Pin 2 ← M3_0 from RTL8763ESE
#define BT_UART_TX_PIN  0       // GP0 = Pico Pin 1
#define BT_BAUD_RATE    115200  // Confirmed via Docklight on M3_0
...
// ─── CAN ──────────────────────────────────────────────────────────────────────
#define CAN_DLC         8
#define BT_FRAME_BYTES  6
#define CAN_SIDH        0xC6    // 0x636 >> 3
#define CAN_SIDL        0xC0    // (0x636 & 7) << 5
...
static uint8_t mcp_read_reg(uint8_t reg) {
    ...
    spi_read_blocking(SPI_PORT, 0x00, &rx[2], 1);
    cs_deselect();
    return rx[2];
}
...
static bool mcp2515_init(void) {
    mcp_reset();
    // 250 kbps @ 8 MHz: CNF1=0x00, CNF2=0xBA, CNF3=0x03
    // 16TQ x 250ns = 4us per bit, 75% sample point
    mcp_write_reg(REG_CNF1, 0x00);
    mcp_write_reg(REG_CNF2, 0xBA);
    mcp_write_reg(REG_CNF3, 0x03);
    mcp_write_reg(REG_CANCTRL, 0x00);
    sleep_ms(10);
    return ((mcp_read_reg(REG_CANSTAT) & 0xE0) == 0x00);
}
...
int main(void) {
    ...
    while (true) {
        ...
        while (uart_is_readable(BT_UART)) {
            buf[buf_len++] = uart_getc(BT_UART);
            if (buf_len == BT_FRAME_BYTES) {
                bool ok = can_send(buf, buf_len);
                if (ok) {
                    frames_sent++;
        ...
        // Flush partial frame after 5 ms silence
        if (buf_len > 0) {
            sleep_ms(5);
            if (!uart_is_readable(BT_UART)) {
                ...
                if (ok) {
                    frames_sent++;
                    printf("[PARTIAL %dB #%u] %02X %02X %02X %02X %02X %02X\n",
                           buf_len, (unsigned)frames_sent,
                           buf_len > 0 ? buf[0] : 0,
                           buf_len > 1 ? buf[1] : 0,
                           buf_len > 2 ? buf[2] : 0,
                           buf_len > 3 ? buf[3] : 0,
