...
#define COOLDOWN_PERIOD_MS  4000    // 4 seconds delay before heartbeat starts
...
int main(void) {
    ...
    printf("[UART->CAN] Heartbeat=%02X @ 10ms when UART silent\n\n", HEARTBEAT_VALUE);
    ...
    // Cooldown management states
    uint32_t       silence_start_time = 0;
    bool           waiting_for_4s     = false;
    ...
    while (true) {
        ...
        while (uart_is_readable(BT_UART)) {
            ...
            last_uart_rx   = now;
            uart_active    = true;
            waiting_for_4s = false; // Reset 4s window if any byte is actively incoming
        ...
        // ── Detect initial UART silence (100ms) ──────────────────────────────────
        if (uart_active && (now - last_uart_rx >= UART_SILENCE_MS)) {
            uart_active = false;
            waiting_for_4s = true;
            silence_start_time = now; 
            printf("[SILENCE DETECTED] UART quiet. Commencing 4-second pause...\n");
        }

        // ── Handle the 4-second wait window ──────────────────────────────────
        if (waiting_for_4s) {
            if (now - silence_start_time >= COOLDOWN_PERIOD_MS) {
                waiting_for_4s = false; // Cooldown expired
                last_heartbeat = now;   // Sync timer to start heartbeat immediately
                printf("[HEARTBEAT RUNNING] 4s passed. Starting 10ms heartbeat.\n");
            }
        }

        // ── Heartbeat: send every 10ms ONLY if UART is silent AND 7s has elapsed ──
        if (!uart_active && !waiting_for_4s && (now - last_heartbeat >= HEARTBEAT_PERIOD_MS)) {
