...
#define MCP_SPI_BAUD        1000000u    /* 1 MHz — safe, matches working reference */
...
static void can_send(uint8_t signal_value)
{
    /* Clear TXREQ from any previous send before loading new frame */
    mcp_write_reg(MCP_REG_TXB0CTRL, 0x00);

    /* Write SIDH, SIDL, EID8, EID0, DLC in one sequential SPI transaction */
    ...
    /* Write 8 data bytes: signal_value in byte 0, zeros in bytes 1-7 */
    uint8_t payload[8];
